...
const ResultsTable = ({ runners, modalities, genders, eventId, title }: { runners: Runner[], modalities: Modality[], genders: Gender[], eventId?: string, title?: string }) => {
    ...
    const filteredRunners = useMemo(() => {
        ...
        return filtered
    }, [runners, search, selectedModality, selectedCategories, selectedGender])

    const filtersCount = useMemo(() => {
        return [selectedModality, selectedCategories.length > 0, selectedGender].filter(Boolean).length
    }, [selectedModality, selectedCategories, selectedGender])
    ...
    return (<div className={`modern-table w-full custom_border flex flex-col rounded-xl h-full `}>
        <div className="flex flex-col border-b border-gray-200 dark:border-cgray">
            ...
            <div className="px-4 pb-1 flex items-center justify-between">
                <div className={`text-xs text-gray-400 dark:text-gray-500 ${filtersOpen ? 'opacity-100' : 'opacity-0'} transition-all`}>
                    Filtros
                </div>
            ...
            <table className="w-full">
                <thead className="modern-table-header">
                    <tr>
                        ...
                        <th className="!w-[50px] align-middle font-medium text-gray-700 dark:text-gray-300 text-center">
                            Dorsal
                        </th>
